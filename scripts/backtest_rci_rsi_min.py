...
def patch(text: str, *, rci_exit: int, rsi_min: float) -> str:
    text = patch_config_past_period(text, PAST_DAYS)
    text = re.sub(r"SCR_JDG_RCI_EXIT\s*=\s*\d+", f"SCR_JDG_RCI_EXIT = {rci_exit}", text)
    text = re.sub(r"SCR_RCI_EXIT_RSI_MIN\s*=\s*[\d.]+", f"SCR_RCI_EXIT_RSI_MIN = {rsi_min}", text)
    if "SCR_RCI_EXIT_RSI_MIN" not in text:
        text = text.replace(
            "SCR_RCI_EXIT_PROFIT_ONLY = 0",
            f"SCR_RCI_EXIT_PROFIT_ONLY = 0\nSCR_RCI_EXIT_RSI_MIN = {rsi_min}",
        )
    text = re.sub(r"SCR_RCI_EXIT_PROFIT_ONLY\s*=\s*\d+", "SCR_RCI_EXIT_PROFIT_ONLY = 0", text)
...
def run(name: str, rci_exit: int, rsi_min: float, cache_rel: str | None) -> dict:
    cache = ROOT / cache_rel if cache_rel else ROOT / "output" / "optimize" / f"trades_RCI_rsi{int(rsi_min)}.json"
    if cache.is_file() and cache_rel:
        ...
        text = patch(base, rci_exit=rci_exit, rsi_min=rsi_min)
        tmp = Path(tempfile.mkdtemp(prefix="kaburadar3-rci-rsi-"))
...
def main() -> int:
    rows = []
    for name, rci, rsi_min, cache in VARIANTS:
        print(f"running {name}...", flush=True)
        rows.append(run(name, rci, rsi_min, cache))

    print("\n=== RCI決済 + RSI_MIN グリッド ===")
    print(f"{'設定':<12} {'10年':>10} {'2020除':>10} {'PF':>6} {'勝率':>6} {'RSI60':>6} {'損切':>5} {'その他':>5} {'勝中央':>7} {'負中央':>7}")
    for r in sorted(rows, key=lambda x: -x["ex2020"]["total"]):
        a, x = r["all"], r["ex2020"]
        print(
            f"{r['name']:<12} {a['total']:>+10,} {x['total']:>+10,} {x['pf']:>6} "
            f"{x['win']:>5.1f}% {x['rsi60']:>6} {x['stops']:>5} {x['other']:>5} "
            f"{x['med_w']:>+7,} {x['med_l']:>+7,}"
        )
    best = max(rows, key=lambda x: x["ex2020"]["total"])
    base = next(r for r in rows if r["name"] == "現行OFF")
    rci = next(r for r in rows if r["name"] == "RCI20/5")
    print(f"\n2020除 best: {best['name']} ({best['ex2020']['total']:+,})")
    print(f"  vs 現行: {base['ex2020']['total']:+,}  vs RCI20/5: {rci['ex2020']['total']:+,}")
    return 0
